...
**λ 函数**也即匿名函数，在 Rust 中体现为**闭包**。在一些语言中，你可以简单地在 λ 函数内调用自己实现递归，例如在 JavaScript 中实现一个阶乘：
...
`.` 前为参数定义，`.` 后为函数体，在无歧义的情况下可以省略括号。例如简单地计算 `f(x) = x + 2` 可以给出 λ 演算：`λx.x + 2`。
...
α 等价意味着参数的命名不影响结果，也就是说 `λx.x + 2` 与 `λy.y + 2` 完全等效，这也符合我们的编程逻辑：
...
β 规约实际上等同于将函数调用时的参数替换到未知数上，例如将 `(λx.x + 2) 3` 规约后就得到 `3 + 2`，这意味着 `(λx.x + 2) 3` 等价于 `3 + 2`。
...
在 λ 演算中，我们不知道函数的名字，我们用 `#` 来表示匿名，则：

```text
fact(x) = (x == 0 ? 1 : x * #(x - 1))
```

我们的目的是让 `fact` 函数的函数体内不要出现 `#`。因此，我们将上述代码包装为 `g(f)` ：
...
此时，在 `f'` 的定义中，`Y` 和 `g` 都已知，我们成功消除了匿名，得到了 `f'`，而 `f'` 就是我们需要的 `fact` 函数：

```text
fact(x) = Y(g)(x)
```

现在就是最后一个问题，是否真的存在一个不动点算子 Y 使得对于任何函数 `g(f)` 和其不动点 `f'` 都有 `f' = Y(g)`？答案是**存在**，直接说结论：
...
我们遇到阻碍的根本原因是，Rust 中确定一个类型不能取决于类型本身，这会导致无限展开从而无法确定类型的大小。那么我们可以换个思路，使用闭包自身类型的**引用**（准确的说，是 Trait Object 的引用），因为引用的大小是固定的，所以能够确定类型的大小：
...
Boom！全部木大，我们**无法做到将一个具有捕获值的临时闭包的引用传递到 `g` 的外面**（无捕获的临时闭包可以），即使你用 `Func` 也不能改变结果。
